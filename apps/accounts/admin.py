...
@admin.register(User)
class UserAdmin(SafeAdminMixin, ModelAdmin, BaseUserAdmin):
    ...
    def delete_users(self, request, queryset):
        ...
        try:
            ...
            logger.exception("Error in delete_users action: %s", error_msg)
    ...
    def save_model(self, request, obj, form, change):
        ...
        try:
            ...
            if not change and not password_provided:
            ...
            error_msg = '; '.join(e.messages) if hasattr(e, 'messages') else str(e)
            self.message_user(request, error_msg, messages.ERROR)
            logger.exception("Error saving user %s: %s", getattr(obj, 'email', ''), error_msg)
    ...
    def reset_password_action(self, request, queryset):
        ...
        for user in queryset:
            try:
                ...
                self.message_user(
                    request,
                    format_html('<strong>{}</strong>: {}', user.email, error_msg),
                    messages.ERROR
                )
                logger.exception("Error resetting password for %s: %s", user.email, error_msg)
